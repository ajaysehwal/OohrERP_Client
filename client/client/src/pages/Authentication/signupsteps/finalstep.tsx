  ...
  const handlesubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.get(
        `${url}/apiadmindata/${formData.admin_email}`
      );
          ...
          if (
            ...
            if (formData.admin_password.length < 8) {
              ...
              if (formData.admin_password != formData.admin_confirmPassword) {
                ...
                postData(formData,'service_3owuf8a','template_8s6hdb9','zJTNZDHXA0pfWWLqb');
  ...
  const postData = async (formData: any, service_id: any, template_id: any, public_key: any) => {
    ...
    try {
      ...
      const senddata={
        ...
        reply_to: "Info@myoohr.com",
        
        }
        document.location.href = '/signin';

       emailjs.send(service_id, template_id, senddata , public_key).then(
            (result) => {
              if (result.text == 'OK') {
