...
export default function AddSchoolHoilday() {
  ...
  const postclasses=async(formdata:object )=>{
      setload(true)
      try{
          const res=await axios.post('https://oohrerpsystem.onrender.com/schoolholiday',formdata);
  ...
  const getclasses=async(token:any)=>{
    setload(true)
    try{
      const res=await axios.get(`https://oohrerpsystem.onrender.com/schoolholiday/${token}`);
        setclasses(res.data);
